...
def test_failure_message_keeps_the_error_not_just_the_session_line(tmp_path):
    ...
    with pytest.raises(AnalystError, match="provider refused the request"):
        _analyst(Runner([""]), tmp_path).ask("p")
